...
CREATE TYPE statement_status AS ENUM('queued', 'processing', 'parsed', 'complete', 'failed');
...
-- (period_start) rather than by a single statement: a month spans every account
-- the user owns, so one budget per (user, category, month) is the right grain.
CREATE TABLE budgets (
  ...
  amount       NUMERIC(12,2)  NOT NULL,
  UNIQUE (user_id, category_name, period_start),
  FOREIGN KEY (user_id) REFERENCES users(user_id)
    ON UPDATE CASCADE ON DELETE CASCADE,
  FOREIGN KEY (category_id) REFERENCES categories(category_id)
    ON UPDATE CASCADE ON DELETE CASCADE
